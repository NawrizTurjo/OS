...
void stationLock(Operative *operative, string TS, string UNIT, string LEADER)
{
    ...
    if (NOTIFICATION_LOGGING)
        writeOutput("Operative " + to_string(operative->identifier) + UNIT + LEADER + " got access at typewriting station " + TS + "at time " + to_string(getTime()));
...
void stationUnlock(Operative *operative, string TS, string UNIT, string LEADER)
{
    ...
    {
        if (NOTIFICATION_LOGGING)
            writeOutput("Operative " + to_string(id) + UNIT + LEADER + " is avaliable to work at typewriting station " + TS + "at time " + to_string(getTime()));
    }
...
void *operativesWork(void *arg)
{
    ...
    stationUnlock(operative, TS, UNIT, LEADER);
    // semPost(&stationSemaphores[operative->stationNum - 1]);
